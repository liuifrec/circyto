...
- synthetic SComatic candidate table normalization
...
Active or planned server-scale runs:

- IMR90 full 23-cell RNA-side single-end run
- HAP1 batch10 paired-end run
- HAP1 future full 63-cell run

### Synthetic-Only Components

The following are integration scaffolds only:

- synthetic SComatic chr21 proof of concept
- imported `scomatic_candidate_summary.tsv`
- per-cell candidate signal counts in joined summaries
- DNA/SNV and DNA/CNV MuData slots, not yet production exporters
- real SComatic candidate generation from RamDA/scRR alignments
...
These should enter through `dna_cell_summary.tsv` or a future `dna_cnv_summary.tsv` generated by external DNA/CNV pipelines.
...
This is not a DNA modality by itself. It is currently stored under `dna/` because it belongs to the future DNA/SNV integration scaffold, but manuscript language should call it RNA-derived candidate variant evidence.
...
- future DNA SNV and CNV summaries

### MuData Structure

Current production MuData:

- `mdata["rna"]`
- `mdata["circ"]`
- shared `mdata.obs`

Planned multimodal structure:
...
- `mdata["circ"]`
  - circRNA counts
- `mdata["dna_snv"]`
  - external DNA SNVs or candidate signal matrix/table representation
- `mdata["dna_cnv"]`
  - CNV burden or segment-level data
- `mdata.obs`
...
Short term, joined TSV summaries are the stable integration output. Promote candidate variants and DNA state into MuData only after real external outputs are available and the representation is stable.
...
- MuData interoperability figure:
  - `rna`, `circ`, planned `dna_snv`, planned `dna_cnv`
  - shared `obs` schema
...
- adapter output must be tested against a real SComatic Step 1 run outside WSL
- full IMR90/HAP1 server runs need stable snapshots before candidate-signal generation

Data blockers:

- full public dataset FASTQs/alignments must complete
...
- no production parser yet for real SComatic final outputs
- no stable `dna_snv`/`dna_cnv` MuData modality export yet
...
Implement a read-only SComatic result normalizer:

```bash
circyto normalize-scomatic-results \
  --scomatic-output-dir SCOMATIC_RUN_DIR \
  --adapter-summary WORKDIR/scomatic_adapter/adapter_summary.json \
  --out WORKDIR/dna/scomatic_candidate_summary.tsv
```

Requirements:

- parse real SComatic final pass/fail tables after one HPC smoke run
