...
console = Console()
def convert_matrix_files(matrix_path: Path, circ_index_path: Path, cell_index_path: Path, loom: Optional[Path] = None, h5ad: Optional[Path] = None):
    ...
    if loom:
        ...
        row_attrs = {'circ_id': circ}; col_attrs = {'CellID': cells}
    ...
    if h5ad:
        ...
        adata = ad.AnnData(X=M.T)
        adata.var_names = pd.Index(circ, dtype='string'); adata.obs_names = pd.Index(cells, dtype='string')
